...
CreateJointPositionCallback(
  ...
  return [&global_handler, &state_publisher, &model, &current_state_info,
    ...
    try {
      ...
      if (!global_handler->traj_interpolator_ptr || !global_handler->controller_ptr) {
        // Trajectory interpolator or controller not initialized yet, return current position
        std::array<double, 7> current_joint_positions;
        Eigen::VectorXd::Map(&current_joint_positions[0], 7) =
            current_state_info->joint_positions;
        franka::JointPositions output(current_joint_positions);
        return output;
      }
      ...
      global_handler->logger->error("Exception in joint position callback: {}", e.what());
      ...
      global_handler->logger->error("Unknown exception in joint position callback");
      global_handler->running = false;
      std::array<double, 7> current_joint_positions;
      Eigen::VectorXd::Map(&current_joint_positions[0], 7) =
          current_state_info->joint_positions;
